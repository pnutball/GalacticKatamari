...
func _ready():
	...
	$PropertyBox.text = P_Path[P_Property_Formatted()]
	$PropertyBox.text_changed.connect(_on_value_change)
	$PropertyName.tooltip_text = P_Tooltip

func _on_value_change(_value):
	@warning_ignore("incompatible_ternary")
	if P_Path is Dictionary or P_Path is Array: P_Path[P_Property_Formatted()] = $PropertyBox.text
	else: P_Path.set(P_Property_Formatted(), $PropertyBox.text)
	ChangeMade.emit()
