...
const StringBox = preload("res://editor/sub_property_dialogue.tscn")
...
func _on_value_change(_value):
	...
	for entry in $PropertyMargin/PropertyTexts.get_children():
		if entry.get_node("SubPropertyBox").text != "" or entry.get_node("SubPropertyLang").text.to_lower() == "en":
			Texts[entry.get_node("SubPropertyLang").text.to_lower().trim_suffix(":")] = entry.get_node("SubPropertyBox").text.c_unescape().json_escape()
